    ...
    if (!args.length) {
      ...
      return message.author.send(data, { "split": true }).
        then(() => {
          ...
          message.reply('Check your Dms for commands');
        }).
        catch(error => {
          console.error(`Could not send help DM to ${message.author.tag}.\n`, error);
          message.reply(`could not send help DM to ${message.author}!
					best if you find shelter until help arrives.
					when you find a payphone, craft a quarter then ping/DM \`\`\`lamar#6227\`\`\` as i am too lazy/unable to notify my own master`);
        });
    }
